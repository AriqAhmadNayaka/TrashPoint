...
if (isset($_GET['message'])) {
    ...
    if ($_GET["message"] == "successCreateUser") {
        echo "<script>
            Swal.fire({
                title: 'Berhasil buat akun!',
                text: 'Silahkan login login!',
                icon: 'success',
...
<div class="w-full flex flex-col justify-between items-center" style="background-color: #3f3f46; height: 25%;">
    <img src="../../assets/images/trashpointTextWhite.png" height="350px" width="350px" class="mt-20" alt="">
</div>
<div class="w-full flex flex-col items-center" style="height: 75%;">
    ...
    <div class="w-full flex flex-col items-center justify-between" style="height: 80%;">
        <div class="px-14 w-full h-full flex flex-col">
            <h1 class="text-3xl text-center mt-5">Buat akun!</h1>
            <form action="<?= $url ?>/Api/routes.php?action=createUserMasyarakat" method="post" class="w-full mt-5">
                <div class="flex flex-col w-full mt-5 gap-2">
                    ...
                    <input type="email" name="email" class="border border-gray-300 rounded-md p-2 w-full outline-none" placeholder="Email">
                    <input type="password" name="password" class="border border-gray-300 rounded-md p-2 w-full outline-none" placeholder="Password">
                    <input type="text" name="phoneNumber" class="border border-gray-300 rounded-md p-2 w-full outline-none" placeholder="Phone Number">
                    <button type="submit" name="submit" value="submit" class="bg-zinc-700 textWhite rounded-md p-2 w-full">Registrasi</button>
                </div>
            </form>
        </div>
    </div>
    <p class="mb-1">Sudah punya akun? <a href="./login.php" style="color: #3b82f6;">Login !</a></p>
</div>
